...
def write_mol2_file(atoms, filename, a, b, c, alpha, beta, gamma, skip_long_bonds=False):
    ...
    file.write('MOL\n')
    file.write(f'{format_val(len(atoms), 6)}{format_val(len(bonds), 6)}{format_val(1, 6)}{format_val(0, 6)}{format_val(0, 6)}\n')
    file.write('SMALL\n')
    ...
    for atom in atoms:
        ...
        file.write(format_val(atom_idx, 6))
        file.write(' ')
        ...
        file.write('   ')
        file.write(format_val(atom.name, 5, allign_left=True))
    ...
    file.write('     1 RES1        1 GROUP             0 ****  ****    0  \n')
    file.write('@<TRIPOS>CRYSIN\n')
    file.write('   17.8348   17.8348   17.8348   90.0000   90.0000   90.0000     1     1\n')
    file.close()
...
def write_angles(atoms, angle_params, filename, skip_long_bonds=False):
    ...
    for angle in angles:
        ...
        for i in range(len(params)):
            if abs(params[i][1] - angle_val) < abs(best_params[1] - angle_val):
        ...
        file.write(format_val(best_params[0], 7   ))
        file.write(format_val(best_params[1], 7, 3))
        for i in range(2, len(best_params)):
            file.write(format_val(best_params[i], 7, 3))
        file.write('\n')
    file.close()
...
def write_restraints(atoms, selected_types, filename, force):
    file = open(filename, 'w')
